...
// Matman
    ...
    if ( checkent(self) == 1 ) {
       ...
       if (  openborvariant("xpos")+30 > getentityproperty(self, "x")
         ...
         || getentityproperty(self, "direction") == 0 &&  checkwall( getentityproperty(self, "x")+30, getentityproperty(self, "z") ) > getentityproperty(self, "a")
         || getentityproperty(self, "direction") == 1 &&  checkwall( getentityproperty(self, "x")-30, getentityproperty(self, "z") ) > getentityproperty(self, "a")
          )
       {
...
{ //Binds target to self
  ...
  if ( checkent(self) == 1 ) {
    ...
    if ( checkent(target) == 1 //target != NULL()
      && getentityproperty(target, "speed") > 0 )
    {
    ...
    if ( DamageOnLanding > 1 )
    {
      ...
      damageentity(target,self , DamageOnLanding, 0, openborconstant("ATK_NORMAL"));
    }
    if ( directionMode == 1) {
...
void alterallstats(void aentity, int stage, int iDifficulty)
{
  ...
  if ( checkent(aentity) == 1
    ...
    iChance = rand()%100;
    if ( iChance > 98 ) {
      aentity = increasespeedforentity( aentity, 1+(iDifficulty/3) );
      sizeX = sizeX-40;
      sizeY = sizeY+20;
    }
...
void createarraymininormal(int atype)
{
 ...
 if ( enemyMiniBoss == NULL() ) {
   ...
   if ( atype == 12) { file = openfilestream("data/scripts/elists/mb12.txt", 0); }
...
void createarraynormal(int atype)
{
 ...
 if ( enemyNormal == NULL() ) {
   ...
   if ( atype == 1) { file = openfilestream("data/scripts/elists/n1.txt", 0); } else
   if ( atype == 2) { file = openfilestream("data/scripts/elists/n2.txt", 0); } else
   if ( atype == 3) { file = openfilestream("data/scripts/elists/n3.txt", 0); } else
   if ( atype == 4) { file = openfilestream("data/scripts/elists/n4.txt", 0); } else
   if ( atype == 5) { file = openfilestream("data/scripts/elists/n5.txt", 0); } else
   if ( atype == 6) { file = openfilestream("data/scripts/elists/n6.txt", 0); } else
   if ( atype == 7) { file = openfilestream("data/scripts/elists/n7.txt", 0); } else
   if ( atype == 8) { file = openfilestream("data/scripts/elists/n8.txt", 0); } else
   ...
   if ( atype == 12) { file = openfilestream("data/scripts/elists/n12.txt", 0); }
   ////////////////////////This limits names up to a 100 in one file can be change here if more is needed.////////////////////////
...
// EnemyType : Types of enemy, 1 = normal, 2 = Mini Boss, 3 = Boss.
    ...
    if ( vSelf != NULL() && getentityproperty(vSelf, "exists")) {
      ...
      if (allplrhealth > 50) {
        ...
        if (allplrhealth < 10) {
          if ( iDifficulty - 6 < 1) { iDifficulty = 1; } else { iDifficulty = iDifficulty - 6; }
        }
...
void buyminibossenemy(int amount)
{
  ...
  if ( checkent(vSelf) == 1 ) {
    ...
    if (amount == 1) {
      VariatySetMiniBoss++;
      if ( VariatySetMiniBoss > 12) { VariatySetMiniBoss = 1; } } else {
      VariatySetMiniBoss--;
      if ( VariatySetMiniBoss < 1) { VariatySetMiniBoss = 12; }
    }
    ...
    if (VariatySetMiniBoss==12) {
      settextobj(13, 200, 136, 1, 1, "12: Hard Mini Boss SNK Enemy Set. ", openborvariant("elapsed_time")+1000); }
...
void buynormalenemy(int amount)
{
  ...
  if ( checkent(vSelf) == 1 ) {
    ...
    if (amount == 1) {
      VariatySet++;
      if ( VariatySet > 12) { VariatySet = 1; } } else {
      VariatySet--;
      if ( VariatySet < 1) { VariatySet = 12; }
    }
    ...
    if (VariatySet==12) {
      settextobj(13, 200, 136, 1, 1, "12: Hard SNK Enemy Set.", openborvariant("elapsed_time")+1000); }
